...
with open('Transitions/transitions_75k_FRI.pkl', 'rb') as f:
        ...
        if self.number >= 100:
            results = Parallel(n_jobs=16, backend='loky')(
                delayed(vehicle_observe)(x, self.zone_lookup, demand_current, geodistance, current_time,
                                         self.A2CPolicy, exploration_rate)
                for x in self.X)
        else:
...
def vehicle_observe(x, zone_lookup, demand_current, geodistance_function, current_time, A2CPolicy, exploration_rate):
...
def vehicle_decide(state_matrix, A2CPolicy, exploration_rate):
    num_states = state_matrix.shape[0]
    state_matrix = torch.tensor(state_matrix, dtype=torch.float32).to(device)

    if random.random() > exploration_rate:
        actions, q_values = A2CPolicy.get_action(state_matrix)
