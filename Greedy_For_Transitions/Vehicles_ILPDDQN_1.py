# larger lr and tau compared with Vehicles_ILPDDQN
import pandas as pd
import numpy as np
import ast
import folium
...
class Vehicles():
    def __init__(self, num_vehicles):
        ...
        self.optimizer = optim.Adam(self.DQN_training.parameters(), lr=0.001)
        ...
        for i in range(self.number):
            ...
            if feedback is not None:
                ...
                if current_time == episode_time+test_time:
                    self.X_experience[i].append(random.randint(1, 10))
    ...
    def learn(self, CAPACITY=Capacity):
        if len(self.M) == CAPACITY:
            ...
            for m in range(training_temp):
                ...
                train(self.M, self.DQN_training, self.optimizer, self.criterion)
                training_loss = calc_loss(self.M, self.DQN_training, self.criterion).item()
...
def vehicle_decide(state_matrix, DQN_training, exploration_rate):
    ...
    if random.random() > exploration_rate:
        ...
        values_np = 1000 * np.ones(num_states)
        # 转换成torch tensor
