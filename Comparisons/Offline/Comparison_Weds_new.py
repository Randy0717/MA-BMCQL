...
with open('recording_ILPDDQN_1000car_0124_WED_100percent_offline_DDQN.pkl', 'rb') as f:
    loss = pickle.load(f)
...
with open('recording_ILPDDQN_1000car_0124_WED_100percent_offline_cql400.pkl', 'rb') as f3:
...
with open('recording_ILPAC_1000car_0207_100percent_offline_WED_1.pkl', 'rb') as f3:
...
s1 = pd.Series(Validation)[:22]

# Compute rolling statistics for s2
s2 = pd.Series(Validation1)[:22]

s3 = pd.Series(Validation2)[:22]
...
rolling_max3 = s3.rolling(window=1).max()
rolling_min3 = s3.rolling(window=1).min()
...
plt.title('Comparisons in RS for task 1000_CARS_WEDs')
